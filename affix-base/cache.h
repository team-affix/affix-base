...
namespace affix_base
{
	namespace data
	{
		...
		public:
			...
			cache(
				const std::function<bool(LOCAL_TYPE&)>& a_pull,
				const std::function<bool(LOCAL_TYPE&)>& a_push,
				const std::function<bool(LOCAL_TYPE&)>& a_validate,
				LOCAL_TYPE a_local = {},
				const std::function<void(LOCAL_TYPE&)>& a_import_failed_callback = [](LOCAL_TYPE&) {},
				const std::function<void(LOCAL_TYPE&)>& a_export_failed_callback = [](LOCAL_TYPE&) {}
			) :
				m_pull(a_pull),
				m_push(a_push),
			...
			{
				if (!m_pull(m_local) || !m_validate(m_local))
				{
					m_import_failed_callback(m_local);
				}
			}
			...
			{
				if (!m_validate(m_local) || !m_push(m_local))
				{
					m_export_failed_callback(m_local);
				}
			}
		...
		protected:
			...
			/// </summary>
			/// <returns></returns>
			std::function<bool(LOCAL_TYPE&)> m_pull;

			/// <summary>
			/// Pushes to the remote.
			/// </summary>
			/// <returns></returns>
			std::function<bool(LOCAL_TYPE&)> m_push;

			/// <summary>
			/// Validates that the local is in an acceptable form.
			/// </summary>
			/// <returns></returns>
			std::function<bool(LOCAL_TYPE&)> m_validate;

		protected:
			/// <summary>
			/// Callback for when importing fails.
			/// </summary>
			std::function<void(LOCAL_TYPE&)> m_import_failed_callback;

			/// <summary>
			/// Callback for when exporting fails.
			/// </summary>
			std::function<void(LOCAL_TYPE&)> m_export_failed_callback;
